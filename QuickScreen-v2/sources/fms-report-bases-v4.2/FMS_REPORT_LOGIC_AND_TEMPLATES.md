...
Nie wybieraj tekstu wyłącznie po `test_id`.

# 17. Obserwacja ≠ priorytet ≠ działanie

Te same dane mogą zasilać różne komunikaty.

Przykład bólu szyi i barku:
...
# 18. Nagłówki muszą być samowyjaśniające

Preferowane dla raportu bez bólu:

```text
Czy możesz trenować?
Co jest teraz najważniejsze?
Co zrobić po badaniu?
Co ten wynik oznacza dla [sportu]?
...
# 19. Nazwy techniczne dopiero niżej

W górnej części raportu używaj naturalnych nazw. `Shoulder Clearing`, `Toe Touch`, `Cervical Rotation + Extension` itp. mogą pojawić się w `Pełnym obrazie wyników`.

# 20. Agregacja kilku bolesnych wyników

Nie renderuj kilku bolesnych testów jako surowej listy w głównej części. Zbuduj `natural_area_list` oraz `natural_movement_list` i użyj wspólnego szablonu.

# 21. Priorytet wyniku 1

Nie:

```text
Najważniejszy obszar: Toe Touch
```
...
```text
Co jest teraz najważniejsze?
Najpierw skup się na poprawie skłonu i kontroli zgięcia bioder.
```

# 22. Kontekst sportowy

Pierwsze zdanie sekcji musi nazwać problem.
...
# 23. Testy deterministyczne

Dla fixture raportu zapisuj oczekiwane:
