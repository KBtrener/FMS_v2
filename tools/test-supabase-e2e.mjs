...
try{
  adminSql(`insert into auth.users (id,instance_id,aud,role,email,encrypted_password,email_confirmed_at,created_at,updated_at,raw_app_meta_data,raw_user_meta_data) values ('${userIdA}','00000000-0000-0000-0000-000000000000','authenticated','authenticated','${emailA}',crypt('${password}',gen_salt('bf')),now(),now(),now(),'{}','{}'),('${userIdB}','00000000-0000-0000-0000-000000000000','authenticated','authenticated','${emailB}',crypt('${password}',gen_salt('bf')),now(),now(),now(),'{}','{}'); insert into auth.identities (provider_id,user_id,identity_data,provider) values ('${userIdA}','${userIdA}',jsonb_build_object('sub','${userIdA}','email','${emailA}'),'email'),('${userIdB}','${userIdB}',jsonb_build_object('sub','${userIdB}','email','${emailB}'),'email');`);
  ...
  const fields=await ensure(a.from('test_fields').select('*').eq('is_scoring_input',true),'fields');const options=await ensure(a.from('answer_options').select('*'),'options');
  ...
  path=`${sessionA.user.id}/${assessmentId}/e2e.txt`;await ensure(a.storage.from('assessment-files').upload(path,new Blob(['QuickScreen E2E'],{type:'text/plain'}),{upsert:true}),'upload A file');
  ...
  console.log(JSON.stringify({auth:true,clients:true,assessments:true,answers:true,updateRpc:true,rlsTwoUsers:true,storageUploadDownloadDelete:true,archive:true}));
}finally{
