...
# 22. Techniczna nazwa w naturalnej sekcji

Walidacja powinna zgłosić błąd, jeśli w głównej części raportu bez potrzeby pojawia się:
...
Wyjątek: `Pełny obraz wyników`, panel trenera, szczegóły techniczne.

# 23. Niezgodność nagłówka i odpowiedzi

Błąd, jeśli `heading_intent` i `message_intent` są znaczeniowo niezgodne.
...
Pierwsze zdanie sekcji nie powinno zaczynać się od `to`, `ten problem` lub `ten obszar`, jeśli referent znajduje się wyłącznie w innej sekcji.

# 25. Agregacja wielokrotnego bólu

Jeśli ból występuje w dwóch lub więcej testach:
- główna obserwacja = jedno naturalne zdanie,
- techniczne testy = szczegóły niżej.
