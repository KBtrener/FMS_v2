...
## 16. Warstwa semantyczna

Generator buduje `report.findings[]`:
...
priority_order
client_area_label
client_area_genitive
client_area_locative
client_movement_phrase
technical_label
```

## 17. Message plan

Przed renderowaniem raportu utwórz `report.message_plan[]`:
...
`client_language` nie powinien używać technicznych nazw testów, jeśli istnieje naturalny odpowiednik.

## 18. Deterministyczność

Dane wejściowe + wersja baz treści muszą jednoznacznie wyznaczać tekst raportu. Nie używaj generatywnego AI do renderowania treści.
