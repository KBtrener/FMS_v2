...
# 7. Zasada językowa dla sportu

Zdanie sportowe ma odpowiadać na pytanie `gdzie ten problem pojawia się w mojej dyscyplinie?`.

Preferuj konkret:
- `przy serwisie i smeczu`,
- `przy hamowaniu i zmianie kierunku`,
- `przy lądowaniu`,
- `przy pracy ręką nad głową`.

Nie używaj ogólnych zdań typu:
`ten wynik ma znaczenie w dynamicznej pracy całego ciała`,
jeśli baza sportu pozwala podać konkretny ruch.


---

# 8. Kontekst sportowy zaczyna się od wyniku

Tekst sportowy nie może zaczynać się od niejasnego `to`, `ten sam ruch`, `ten obszar`.

Pierwsza fraza powinna nazwać problem, np.:

```text
Słabsza kontrola prawej nogi ma znaczenie w piłce nożnej szczególnie przy hamowaniu, zmianie kierunku i pracy nogi podporowej.
```
